# Faction Tactics 1.0.1 — Install (Ungrull / anyone)
...
**Both must be the same product version (1.0.1).** Schema mismatch logs a loud error and ignores intents.
...
4. Boot once; confirm log: `FactionTactics 1.0.1 loaded (1.0.1 hybrid commander…`.
...
2. Confirm log: `FactionTactics.Client 1.0.1 loaded — owning-client combat executor…`.
3. Join the server that runs matching **1.0.1** server DLL.
...
If you see `[FactionTactics] Intent schema mismatch` — reinstall **both** packs from the same 1.0.1 release. Do not mix 0.3.x / 0.2.x with 1.0.1.
